...
int main(int argc, char **argv) {
    ...
    //Added a for loop to make creating a table easier
    for (int n = 1; n <= 10; n++) {
        ReplacementAlgorithm* lru = new LRU(n);
        ReplacementAlgorithm* fifo = new FIFO(n);
        ...
        std::cout << "Page frames: " << n << std::endl;
        std::cout << "LRU faults  = " << lru->getPageFaultCount() << std::endl;
        std::cout << "FIFO faults = " << fifo->getPageFaultCount() << std::endl << std::endl;
